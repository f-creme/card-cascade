...
function labelAndColor(card: Card): { label: string, colorClass: string } {
    switch (card.kind) {
        ...
        case "second_chance":
            return { label: "2nde chance", colorClass: "bg-violet-600" };
        case "block":
...
export function CardBack({ count, onClick }: { count: number; onClick?: () => void }) {
    return (
        <button
            ...
            disabled={!onClick}
            onClick={onClick}
            className={`flex h-20 w-14 shrink-0 items-center justify-center rounded-lg border-2 border-base-100 bg-neutral 
                text-sm font-bold text-neutral-content shadow sm:h-24 sm:w-16 ${
